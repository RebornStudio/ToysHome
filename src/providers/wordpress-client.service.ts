...
// import {  } from "../assets/icon/country-file.json";
...
export class WordpressClient {
	...
	public updateUser(adminToken: string, userId: string, userUpdateRequest: UserResponse): Observable<UserResponse> {
		const myHeaders = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': adminToken });
		return this.http
	...
	public createUser(adminToken: string, credentials: RegisterRequest): Observable<RegisterResponse> {
		const myHeaders = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': adminToken });
		return this.http.post<RegisterResponse>(this.config.apiBase + 'wp/v2/users', JSON.stringify(credentials), { headers: myHeaders })
			.concatMap(data => {
				return Observable.of(data);
			});
	}
	...
	public categoriesParent(adminToken: string): Observable<Array<Category>> {
		const myHeaders = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': adminToken });
		return this.http
			.get<Array<Category>>(this.config.apiBase + 'wc/v2/products/categories/?order=desc&orderby=count&parent=0&_embed', { headers: myHeaders })
			.concatMap(data => {
				return Observable.of(data);
			});
	}
	...
	public registerForPushNotification(adminToken: string, userId: string, playerId: string): Observable<any> {
		const myHeaders = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': adminToken });
		return this.http
			.post<any>(this.config.apiBase + 'mobile-ecommerce/v1/notification/register/' + userId, JSON.stringify({ 'player_id': playerId }), { headers: myHeaders })
			.concatMap(data => {
				return Observable.of(data);
			});
	}
